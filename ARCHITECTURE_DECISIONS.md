...
| Deterministic citation normalization and validation | Formatting variants are syntax problems, not semantic reasoning problems | Ask the LLM to repair its own citations; accept any citation-looking prose | V1 validity was 80%; stored-answer replay reached 100%, while newly generated v2 answers measured 94.87% because two new formats were non-canonical | Label validity proves only that an ID exists, not that the passage entails the claim |
...
| FastAPI as composition root with direct orchestration | The request sequence is short and readable | Dependency-injection framework, repositories, service containers | Endpoints call one retrieval engine and one generation function; 42 tests cover end-to-end behavior | Output contracts remain dictionary/TypeScript conventions rather than shared generated schemas |
