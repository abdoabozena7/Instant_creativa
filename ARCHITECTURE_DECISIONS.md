...
| Weighted hybrid default: 0.55 BM25 / 0.45 dense | Combine measured complementary signals without a hidden reranker | Dense-only, BM25-only, reciprocal-rank fusion | Development hybrid achieved 100% Recall@1/5 and MRR; v12 Recall@5 was 97.30% and MRR@6 86.04% | Weights and policy boosts were selected on development data and should not be presented as universal constants |
...
| High-precision emergency redirect before retrieval | A current major emergency should not be treated as guideline lookup or sent to generation | Let the model add a disclaimer; broad symptom triage classifier | Positive and benign-control tests verify redirect before embedding/model; the guard is included in the v12 architecture fingerprint | It recognizes a narrow set of explicit immediate emergencies and is not a general triage system |
...
| Claim-unit citation gate with one bounded repair | A valid `[E#]` elsewhere in an answer does not ground an uncited comparison, bullet, or conclusion | Check labels only; append citations mechanically; retry without a limit | V12 claim citation coverage is 98.50% and complete release pass is 94.87%; two residual failures remain withheld | P50/P95 increased to 5.44/19.61 s; coverage still does not prove entailment |
| Withhold generation that fails the citation contract | Showing an uncited or invalid answer beside retrieved passages falsely implies support | Display a warning badge; let the UI infer refusal text | V12 label validity is 100%; claim coverage failures are fail-closed | Valid-but-wrong claims still require independent semantic evaluation |
